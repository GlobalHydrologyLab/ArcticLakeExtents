...
lakes <- read_csv('data/out/AW_GEE_Exports/UCLA_Pick20_3year.csv') %>%
  select(sampID = SampID, area = area_sum, count, year = year_first) %>% mutate(Threshold = 'area20') %>%
  bind_rows(read_csv('data/out/AW_GEE_Exports/UCLA_Pick50_3year.csv') %>%
  select(sampID = SampID, area = area_sum, count, year = year_first) %>% mutate(Threshold = 'area50')) %>%
  bind_rows(read_csv('data/out/AW_GEE_Exports/UCLA_Pick70_3year.csv') %>%
  select(sampID = SampID, area = area_sum, count, year = year_first) %>% mutate(Threshold = 'area70')) %>%
...
lakes.filtered <- lakes %>% group_by(ID) %>%
  summarise(area70 = mean(area70, na.rm = T)) %>%
  filter(area70 < 10)

lakes <- lakes %>% filter(ID %in% lakes.filtered$ID)
################
## Check out some random individual timeseries
lakeSamp = unique(lakes$sampID) %>% sample(.,20)

lakes %>% filter(sampID %in% lakeSamp) %>%
  select(sampID, year, area20,area50,area70,area90) %>%
...
sampMeans <- lakes %>% group_by(sampID,year) %>%
  summarise_at(vars(area20,area50,area70,area90),sum, na.rm = T) %>%
  group_by(sampID) %>%
  summarise_at(vars(area20,area50,area70,area90),  mean, na.rm = T) %>%
  ungroup() %>%
  pivot_longer(-sampID, names_to = 'Threshold', values_to = 'Mean_Area')

tp <- lakes %>% select(sampID, year, area20,area50,area70,area90) %>%
  pivot_longer(-c(sampID,year), names_to = 'Threshold', values_to = 'area') %>%
  na.omit() %>%
  group_by(sampID, year, Threshold) %>%
  summarise(count = n(),
