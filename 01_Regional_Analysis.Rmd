...
```{r cars}
# Pickens dataset 'data/out/ArcticLakes_UCLA_Pickens.csv'
# Peckel dataset 'data/out/Lakes_UCLA.csv')'
lakes <- read_csv('data/out/ArcticLakes_UCLA_Pickens.csv') %>%
  select(ID = label, pf = pf_first, sampID = first, year = year_first, 
         area = area_sum, tempS = temp_mean, precipS = precip_mean, lith = lith_first, .geo) %>%
...
write_feather(lakes, 'data/out/lakes_UCLA_munged_Pickens.feather')
...
sampCharSF <- samps.sf %>% inner_join(lakeChar %>% ungroup() %>% distinct(sampID, .keep_all = T)) %>% st_as_sf()
